...
def save_to_favorites(cur, conn, fname, lname, link):
    ...
    cur.execute("SELECT id FROM people_found WHERE first_name = %s AND last_name = %s", (fname, lname))
    person_id = cur.fetchone()[0]
    # Вставить новую запись в таблицу favorites
    cur.execute("INSERT INTO favorites (vk_link) VALUES (%s) RETURNING favorit_id", (link,))
    favorit_id = cur.fetchone()[0]
    # Обновить запись в таблице people_found
    cur.execute("UPDATE people_found SET favorit_id = %s WHERE id = %s", (favorit_id, person_id))
    conn.commit()
...
def save_to_black_list(cur, conn, fname, lname, link):
    ...
    cur.execute("INSERT INTO black_list (vk_link) VALUES (%s) RETURNING black_list_id", (link,))
    black_list_id = cur.fetchone()[0]
    # Обновить запись в таблице people_found
    cur.execute("UPDATE people_found SET black_list_id = %s WHERE id = %s", (black_list_id, person_id))
    conn.commit()
